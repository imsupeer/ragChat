...
def test_llama_cpp_provider_resolves_from_settings():
...
def test_llama_cpp_provider_capabilities():
    provider = resolve_llm_provider(Settings(llm_provider="llama_cpp"))
    capabilities = provider.capabilities
    assert capabilities.chat is True
    assert capabilities.streaming is True
    assert capabilities.preload is True
    assert capabilities.unload is False
    assert capabilities.keep_alive is False
    assert capabilities.openai_compatible is True
